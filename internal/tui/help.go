...
var commandHelpEntries = []commandHelp{
	{"Inspection", "paste", "Preview or attach pasted text", "Pastes longer than 800 characters or three lines become indivisible chips. Full original text, including newlines, is sent by default. Ctrl+P opens the paste preview for the composer or a question's Other input. Up/Down selects a paste, PgUp/PgDn scrolls its preview, t sends full text, f explicitly uploads a private text file to this session's durable runtime storage, d removes it from the draft, and i inserts a cached paste into the composer. File mode sends a path and an instruction to read it, not the full text. Escape closes the preview. /paste also lists this session's cached pastes. Login codes and secret answers are excluded. Maximum one paste/file is 1 MiB; the in-memory paste cache is bounded at 32 MiB and is lost on detach. Uploaded files remain with session data even if their draft chip is deleted.", "/paste\n/help paste"},
