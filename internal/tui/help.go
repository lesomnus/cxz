...
var commandHelpEntries = []commandHelp{
	...
	{"Session", "restart", "Restart this session's agent", "Submit /restart, then /restart confirm within 30 seconds for the same session/run. /restart cancel cancels confirmation. Stops the active turn and clears pending approvals, then resumes the same session with its stored history and authentication. Automatic approval resets to manual. Failed stop never proceeds to resume; failed resume is reported without resending prompts. This does not update the manager/runtime or recreate the container. Older shared-profile sessions may require a new session and independent login.", "/restart\n/restart confirm\n/restart cancel"},
