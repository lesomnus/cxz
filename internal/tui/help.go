...
var commandHelpEntries = []commandHelp{
	{"Settings", "settings", "Shared Docker status and maintenance", "Ctrl+P opens settings from the conversation or project list. Shows the shared engine status and build cache usage, refreshed every ten seconds. Start applies the manager’s saved settings. Stop and cache cleanup require confirmation; cleanup removes unused build cache only. Images and volumes are retained. Esc or Ctrl+P returns. Edit configuration with cxz edit.", "/settings"},
