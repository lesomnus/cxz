...
var commandHelpEntries = []commandHelp{
	...
	{"Permissions", "permission", "Choose manual or automatic approval", "ask requires manual approval. full automatically approves supported tool requests for the viewed session run, including pending ones; questions and unknown request types still need a reply. Full approval can allow file changes and shell commands. The setting survives project/session navigation in this TUI. It resets on actual disconnect, run change or restart and does not change provider sandbox policy. Sessions not currently viewed are not automatically approved by this TUI.", "/permission full\n/permission ask"},
	...
	{"Inspection", "details", "Inspect the latest tool result", "Shows the full latest tool result locally instead of its compact transcript summary. Click a Write/Edit row to preview its highlighted input; scroll with the mouse wheel and close with ×. Earlier raw events remain available through the cxz session events CLI command.", "/details"},
