...
func TestTurnSummary(t *testing.T) {
	...
	for _, want := range []string{"↑ 1234", "↓ 321", "↺ 100", "$0.0123 USD", "◷ 2.4s"} {
	...
	for _, line := range strings.Split(text, "\n") {
		if ansi.StringWidth(line) != 100 {
			t.Fatal("not right aligned", line)
		}
	}
...
func TestConversationAlignmentAndSummaryPlacement(t *testing.T) {
	...
	if !strings.Contains(transcript, "reply\n") || strings.Contains(transcript, "reply\n\n") {
		t.Fatal("summary not immediately below reply", transcript)
	}
