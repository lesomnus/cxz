...
func (s *Server) watchRemote(r *api.WatchRequest, stream grpc.ServerStreamingServer[api.Event]) error {
	conn, client, err := s.manager.ClientFor(stream.Context(), r.SessionId)
	if err != nil {
		return err
	}
	defer conn.Close()
	remote, err := client.Watch(stream.Context(), r)
	if err != nil {
		return err
	}
	for {
		...
		if err := s.manager.CacheEvents(stream.Context(), &api.EventBatch{Events: []*api.Event{e}}); err != nil {
			return err
		}
