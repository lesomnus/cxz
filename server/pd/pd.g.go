...
func dispatch(ctx context.Context, s resource.Server, op *pdpb.Op) (*anypb.Any, error) {
	...
	case resource.ProjectService_Devcontainer_FullMethodName:
		v := &resource.DevcontainerRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Project().Devcontainer(ctx, v)
		if err != nil {
			return nil, err
		}
	...
	case resource.SessionService_Permission_FullMethodName:
		v := &resource.SessionPermissionRequest{}
		if err := op.GetRequest().UnmarshalTo(v); err != nil {
			return nil, batch.ErrRequest(m, err)
		}

		res, err := s.Session().Permission(ctx, v)
		if err != nil {
			return nil, err
		}
