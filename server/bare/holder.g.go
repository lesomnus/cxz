...
type HolderServiceServer struct {
	...
	resource.UnimplementedHolderServiceServer
}
...
func NewHolderServiceServer(db *ent.Client, opts ...Option) resource.HolderServiceServer {
...
func (s HolderServiceServer) Add(ctx context.Context, req *resource.HolderAddRequest) (*resource.Holder, error) {
	...
	ds := make([]func(v *resource.Holder), 0, 1)
	...
	if v, err := mint(ctx, s.Mint, "cxz.v2.Holder", k, req.HasId()); err != nil {
		return nil, err
	} else {
		q.SetId(v)
	}
	if k, err := TenantGetKey(ctx, st.Db, req.GetTenant()); err != nil {
		...
		q.SetTenantId(k)
		ds = append(ds, func(v *resource.Holder) {
			v.SetTenant(resource.Tenant_builder{Id: k[:]}.Build())
		})
	}
	...
	if err := record(ctx, s.Rec, st.Db, Change{
		By:  resource.HolderService_Add_FullMethodName,
		Key: u.Id,
	}); err != nil {
		return nil, err
	}
...
func (s HolderServiceServer) Get(ctx context.Context, req *resource.HolderGetRequest) (*resource.Holder, error) {
...
func HolderSelectedFields(m *resource.HolderSelect) []string {
	if m.GetAll() {
...
func HolderSelect(q *ent.HolderQuery, m *resource.HolderSelect) {
	if !m.GetAll() {
...
func HolderSelectInit(q *ent.HolderQuery, m *resource.HolderSelect) {
	if m != nil {
		HolderSelect(q, m)
	} else {
...
func (s HolderServiceServer) Patch(ctx context.Context, req *resource.HolderPatchRequest) (*resource.Holder, error) {
	...
	return s.apply(ctx, req.GetRef(), doc, resource.HolderService_Patch_FullMethodName)
}

func HolderGetKey(ctx context.Context, db *ent.Client, ref *resource.HolderRef) (uuid.UUID, error) {
	var z uuid.UUID
	if ref.HasId() {
...
var holderOrmEntity = ormpatch.MustEntityOf(resource.File_cxz_v2_payday_holder_proto, "Holder")
...
func (s HolderServiceServer) Apply(ctx context.Context, req *resource.HolderApplyRequest) (*resource.Holder, error) {
	if !req.HasPatch() {
		return nil, status.Errorf(codes.InvalidArgument, "%s", ormpatch.ErrNoPatch)
	}
	return s.apply(ctx, req.GetRef(), req.GetPatch(), resource.HolderService_Apply_FullMethodName)
}

func (s HolderServiceServer) apply(ctx context.Context, ref *resource.HolderRef, doc *patchpb.Patch, by string) (*resource.Holder, error) {
...
func (s HolderServiceServer) Erase(ctx context.Context, req *resource.HolderRef) (*resource.HolderEraseResponse, error) {
	...
	if s.Rec != nil {
		...
		if err != nil {
			if ent.IsNotFound(err) {
				return &resource.HolderEraseResponse{}, nil
			}
			return nil, err
		}
	...
	if n > 0 {
		if err := record(ctx, s.Rec, st.Db, Change{
			By:  resource.HolderService_Erase_FullMethodName,
			Key: k,
		}); err != nil {
			return nil, err
		}
	...
	res := &resource.HolderEraseResponse{}
	res.SetErased(n > 0)
...
// naming its parent.
func HolderPick(req *resource.HolderRef) (predicate.Holder, error) {
...
func pickHolder(req *resource.HolderRef) (predicate.Holder, error) {
	switch req.WhichKey() {
	case resource.HolderRef_Id_case:
		if v, err := entuuid.FromBytes(req.GetId()); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "id: %s", err)
		} else {
			return holder.IdEQ(v), nil
		}
	case resource.HolderRef_Slug_case:
		k := req.GetSlug()
		...
		return holder.And(ps...), nil
	case resource.HolderRef_Key_not_set_case:
