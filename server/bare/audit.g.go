...
type AuditServiceServer struct {
	...
	resource.UnimplementedAuditServiceServer
}
...
func NewAuditServiceServer(db *ent.Client, opts ...Option) resource.AuditServiceServer {
...
func (s AuditServiceServer) Add(ctx context.Context, req *resource.AuditAddRequest) (*resource.Audit, error) {
	...
	if v, err := mint(ctx, s.Mint, "cxz.v2.Audit", k, req.HasId()); err != nil {
		return nil, err
	} else {
		q.SetId(v)
	}
	...
	if err := record(ctx, s.Rec, st.Db, Change{
		By:  resource.AuditService_Add_FullMethodName,
		Key: u.Id,
	}); err != nil {
		return nil, err
	}
...
func (s AuditServiceServer) Get(ctx context.Context, req *resource.AuditGetRequest) (*resource.Audit, error) {
...
func AuditSelectedFields(m *resource.AuditSelect) []string {
	if m.GetAll() {
...
func AuditSelect(q *ent.AuditQuery, m *resource.AuditSelect) {
	if !m.GetAll() {
...
func AuditSelectInit(q *ent.AuditQuery, m *resource.AuditSelect) {
	if m != nil {
		AuditSelect(q, m)
	} else {
	}
...
func (s AuditServiceServer) Patch(ctx context.Context, req *resource.AuditPatchRequest) (*resource.Audit, error) {
	...
	return s.apply(ctx, req.GetRef(), doc, resource.AuditService_Patch_FullMethodName)
}

func AuditGetKey(ctx context.Context, db *ent.Client, ref *resource.AuditRef) (uuid.UUID, error) {
	var z uuid.UUID
	if ref.HasId() {
...
var auditOrmEntity = ormpatch.MustEntityOf(resource.File_cxz_v2_payday_audit_proto, "Audit")
...
func (s AuditServiceServer) Apply(ctx context.Context, req *resource.AuditApplyRequest) (*resource.Audit, error) {
	if !req.HasPatch() {
		return nil, status.Errorf(codes.InvalidArgument, "%s", ormpatch.ErrNoPatch)
	}
	return s.apply(ctx, req.GetRef(), req.GetPatch(), resource.AuditService_Apply_FullMethodName)
}

func (s AuditServiceServer) apply(ctx context.Context, ref *resource.AuditRef, doc *patchpb.Patch, by string) (*resource.Audit, error) {
...
func (s AuditServiceServer) Erase(ctx context.Context, req *resource.AuditRef) (*resource.AuditEraseResponse, error) {
	p, err := AuditPick(req)
	if err != nil {
		return nil, err
	}
	...
	if s.Rec != nil {
		...
		if err != nil {
			if ent.IsNotFound(err) {
				return &resource.AuditEraseResponse{}, nil
			}
			return nil, err
		}
	...
	if n > 0 {
		if err := record(ctx, s.Rec, st.Db, Change{
			By:  resource.AuditService_Erase_FullMethodName,
			Key: k,
		}); err != nil {
			return nil, err
		}
	...
	res := &resource.AuditEraseResponse{}
	res.SetErased(n > 0)

	return res, nil
}

func AuditPick(req *resource.AuditRef) (predicate.Audit, error) {
	switch req.WhichKey() {
	case resource.AuditRef_Id_case:
		if v, err := entuuid.FromBytes(req.GetId()); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "id: %s", err)
		} else {
			return audit.IdEQ(v), nil
		}
	case resource.AuditRef_Key_not_set_case:
