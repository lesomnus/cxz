...
type TenantServiceServer struct {
	...
	resource.UnimplementedTenantServiceServer
}
...
func NewTenantServiceServer(db *ent.Client, opts ...Option) resource.TenantServiceServer {
...
func (s TenantServiceServer) Add(ctx context.Context, req *resource.TenantAddRequest) (*resource.Tenant, error) {
	...
	if v, err := mint(ctx, s.Mint, "cxz.v2.Tenant", k, req.HasId()); err != nil {
		return nil, err
	} else {
		q.SetId(v)
	}
	...
	if err := record(ctx, s.Rec, st.Db, Change{
		By:  resource.TenantService_Add_FullMethodName,
		Key: u.Id,
	}); err != nil {
		return nil, err
	}
...
func (s TenantServiceServer) Get(ctx context.Context, req *resource.TenantGetRequest) (*resource.Tenant, error) {
...
func TenantSelectedFields(m *resource.TenantSelect) []string {
	if m.GetAll() {
...
func TenantSelect(q *ent.TenantQuery, m *resource.TenantSelect) {
	if !m.GetAll() {
...
func TenantSelectInit(q *ent.TenantQuery, m *resource.TenantSelect) {
	if m != nil {
		TenantSelect(q, m)
	} else {
	}
...
func (s TenantServiceServer) Patch(ctx context.Context, req *resource.TenantPatchRequest) (*resource.Tenant, error) {
	...
	return s.apply(ctx, req.GetRef(), doc, resource.TenantService_Patch_FullMethodName)
}

func TenantGetKey(ctx context.Context, db *ent.Client, ref *resource.TenantRef) (uuid.UUID, error) {
	var z uuid.UUID
	if ref.HasId() {
...
var tenantOrmEntity = ormpatch.MustEntityOf(resource.File_cxz_v2_payday_tenant_proto, "Tenant")
...
func (s TenantServiceServer) Apply(ctx context.Context, req *resource.TenantApplyRequest) (*resource.Tenant, error) {
	if !req.HasPatch() {
		return nil, status.Errorf(codes.InvalidArgument, "%s", ormpatch.ErrNoPatch)
	}
	return s.apply(ctx, req.GetRef(), req.GetPatch(), resource.TenantService_Apply_FullMethodName)
}

func (s TenantServiceServer) apply(ctx context.Context, ref *resource.TenantRef, doc *patchpb.Patch, by string) (*resource.Tenant, error) {
...
func (s TenantServiceServer) Erase(ctx context.Context, req *resource.TenantRef) (*resource.TenantEraseResponse, error) {
	...
	if s.Rec != nil {
		...
		if err != nil {
			if ent.IsNotFound(err) {
				return &resource.TenantEraseResponse{}, nil
			}
			return nil, err
		}
	...
	if n > 0 {
		if err := record(ctx, s.Rec, st.Db, Change{
			By:  resource.TenantService_Erase_FullMethodName,
			Key: k,
		}); err != nil {
			return nil, err
		}
	...
	res := &resource.TenantEraseResponse{}
	res.SetErased(n > 0)

	return res, nil
}

func TenantPick(req *resource.TenantRef) (predicate.Tenant, error) {
	switch req.WhichKey() {
	case resource.TenantRef_Id_case:
		if v, err := entuuid.FromBytes(req.GetId()); err != nil {
			return nil, status.Errorf(codes.InvalidArgument, "id: %s", err)
		} else {
			return tenant.IdEQ(v), nil
		}
	case resource.TenantRef_Alias_case:
		return tenant.AliasEQ(req.GetAlias()), nil
	case resource.TenantRef_Key_not_set_case:
