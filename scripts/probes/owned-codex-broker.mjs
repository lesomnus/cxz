...
async function until(id,state){for(let i=0;i<100;i++){const s=await json('get',id);if(s.state===state)return s;await delay(100);}throw Error(`timeout ${state}`);}
async function stop(s){await cli('stop',s.id);return until(s.id,'stopped');}
...
try{
 ...
 let a=await json('new','--account','work','--no-attach',projects[0].alias);
 let b=await json('new','--account','work','--no-attach',projects[1].alias);
 ...
 await cli('send',a.id,'approval');let pending=await until(a.id,'waiting_input');await cli('reply',a.id,pending.pending[0].request_id,'allow');await until(a.id,'idle');
 await cli('send',a.id,'wait');await until(a.id,'working');await cli('interrupt',a.id);await until(a.id,'idle');
 a=await stop(a);
 let personal=await json('new','--account','personal','--no-attach',projects[0].alias);await chat(personal,'refresh');personal=await stop(personal);
 a=await json('resume',a.id);assert.equal(a.account,'work');await chat(a);
 ...
 a=await json('recreate','--yes','--account','work','--no-attach',projects[0].alias);assert.equal(a.auth_binding,before);await chat(a,'refresh');
 console.log('manager restart, project recreation, approvals, interrupt and account-preserving resume');
 const inventory=(await json('projects')).projects;
 ...
 assert.equal((await json('account','bindings','work')).items.length,2);
...
}finally{for(const p of projects){await cli('down',p.id);}console.log('scratch workspaces retained; uninstall disposable manager and remove its owned volumes');}
