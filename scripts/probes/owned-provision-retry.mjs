...
async function cli(...args){return (await exec(binary,['--state',state,'--format','json',...args],{timeout:180000,maxBuffer:8*1024*1024})).stdout.trim();}
...
try {
 ...
 const interrupted=await until(async()=>{const p=JSON.parse(await cli('project','ls')).projects.find(p=>p.id===project.id);return p?.provision_state==='interrupted'&&p;});
 ...
 await cli('project','logs',project.id);pass('provisioning log accessible by project ID');
 await cli('project','down',project.id);project=null;
} finally {
 if(project)await cli('project','down',project.id);
