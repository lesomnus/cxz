...
async function cli(...args){return JSON.parse((await exec(binary,['--state',state,'--format','json',...args],{timeout:300000,maxBuffer:8*1024*1024})).stdout);}
...
let restored;for(let i=0;i<60;i++){try{restored=await cli('project','ls');break;}catch{await delay(250);}}
assert(restored,'manager did not restart');assert.deepEqual(restored.projects.filter(p=>p.id).map(p=>p.id).sort(),ids);
const current=await cli('session','get',after.id);assert.equal(current.run_id,after.run_id);assert.equal(current.vendor_id,after.vendor_id);
