...
function cli(name, ...args) {
  return JSON.parse(docker(['exec', name, '/app/bin/cxz', '--state', '/cxz-state', ...args]));
}
async function ready(name) {
  for (let i=0;i<80;i++) { try { cli(name, 'ls');return; } catch { await delay(100); } }
  throw new Error('daemon did not become ready');
}
async function untilState(name,id,state) {
  for(let i=0;i<80;i++){const s=cli(name,'get',id);if(s.state===state)return s;await delay(100);}
...
try {
  ...
  removeContainer(name);name=start(2);await ready(name);s=cli(name,'get',id);
  assert.equal(s.state,'interrupted');assert.equal(s.vendor_id,vendor);assert.ok(s.last_seq>=last);assert.ok(!s.pending?.length);
  check('container_recreation_preserves_journal_registry_and_marks_interrupted');
  s=cli(name,'resume',id);assert.notEqual(s.run_id,oldRun);assert.equal(s.vendor_id,vendor);assert.equal(s.state,'idle');
  assert.throws(()=>cli(name,'reply',id,request,'allow'));await delay(300);assert.equal(cli(name,'get',id).state,'idle');
  check('explicit_resume_new_run_stale_approval_rejected_no_prompt_replay');
  cli(name,'send',id,'after recreation');await untilState(name,id,'idle');
  cli(name,'send',id,'wait');s=await untilState(name,id,'working');removeContainer(name);name=start(3);await ready(name);
  assert.equal(cli(name,'get',id).state,'interrupted');s=cli(name,'resume',id);assert.equal(s.vendor_id,vendor);
  cli(name,'send',id,'after active turn loss');await untilState(name,id,'idle');cli(name,'stop',id);await untilState(name,id,'stopped');
