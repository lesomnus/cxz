...
- 비밀 모달은 composer를 비우지 않는다. 확인하면 선택한 명령 범위만 chip으로 교체하고
  커서를 chip 뒤로 옮긴다. 취소하면 원래 문장·커서가 보존된다. 여러 줄/한글/뒤쪽 문장을 테스트했다.
- 이메일·백틱 내부·붙여넣기는 자동 활성화하지 않는다. 목록이 없으면 Enter는 기존 줄바꿈이다.
- `/redact`를 명령 목록에서 제거하고 도움말을 Inline commands의 `@redact`로 옮겼다.
  비밀 tmpfs 전송·원문 미리보기 금지·수명 정책은 그대로 유지한다.
- 전체 Go 테스트·vet·TUI race 검사 통과. 기존 tmpfs 구성이 있으면 TUI 바이너리 업데이트만 필요하다.
...
- 긴 URL의 표시 줄마다 원본 전체 URL을 OSC 8 대상으로 유지해 중간 줄에서도 링크를 열 수 있다.
- 로그인 화면에서 Ctrl+Y로 전체 원본 URL의 OSC 52 clipboard 복사를 요청한다.
  터미널 지원/허용이 필요하며 성공 여부를 확인한 것처럼 표시하지 않는다. 인증 코드는 복사하지 않는다.
- provider 출력 escape는 계속 제거한다. URL이 출력 chunk 중간에서 잘렸을 때는 완성 전까지
  링크 활성화·복사하지 않는다. 공백·줄바꿈 없는 원본 query를 유지하는 회귀 테스트를 추가했다.
- 전체 Go 테스트·vet·TUI race 검사 통과. 실제 사용 터미널의 OSC 8/52 지원 여부는 별도 확인이 필요하다.
...
- TUI에서 기존 프로젝트 메타데이터와 프로젝트/컨테이너/사용자별 stdio 연결을 재사용한다.
  경로 조회마다 발생하던 전체 프로젝트 RPC/reconcile/inspect/exec를 제거했다(검증/exec는 최초 연결 시).
- 점진 결과/선택 안정성을 유지하고, 취소된 응답 drain 및 deadline 연결 종료, 죽은 연결 재접속을 구현했다.
- 외부 포트/credential/root 권한 없이 remote user로 실행한다. 터미널과 에이전트 프로세스는 변경하지 않았다.
- 전체 테스트·vet·변경 패키지 race 검사 통과. 실제 Docker에서 연결 재사용/재연결/비-root 및 소유권 거부 통과.
- Docker fixture 재사용 경로 조회 약 0.35ms(기존 inspect+exec 약 63ms). 최초 연결 약 68ms.
  전체 프로젝트 조회 및 UI 렌더링 제외 측정이다. docs/wisp.md에 수명/프로토콜/적용 조건을 기록했다.

## 2026-09-13 — 경로 word delete 경계 동작 수정 및 지연 원인 확인

- `/` 바로 뒤에서 word delete하면 구분자와 앞 경로 이름을 다음 경계까지 함께 삭제한다.
  일반 경로 이름 끝에서는 기존처럼 `/` 직전에서 멈춘다. 한글/중간 커서/주변 문장 보존 회귀 테스트 추가.
- 지연 구조 확인: 디렉터리 변경마다 Projects → InspectForeign → manager.Projects를 호출하여
  전체 프로젝트 reconcile/컨테이너 상태 조회를 거친 뒤 별도 inspect와 exec로 경로를 읽고 있었다.
  지속 실행 중인 셸의 Tab 완성과 달리 매번 RPC/프로세스 시작 비용이 발생한다.
  이번에는 원인 확인만 했으며 조회 경량화/연결 재사용은 아직 적용하지 않았다.

## 2026-09-13 — 경로 힌트 점진 조회와 입력 개선

- 디렉터리/파일/실행 파일/심볼릭 링크를 색으로 구분하고 링크 대상 및 fuzzy 매칭 문자 강조를 추가했다.
- Enter는 선택 경로를 완성하고 닫는 백틱을 넣는다. 기존 닫는 백틱은 중복하지 않으며 주변 문장을 보존한다.
- Docker stdout을 NUL 프레임 단위로 읽어 첫 항목부터 전달한다. 추가 batch는 현재 선택과 다음 항목까지 고정하고
  이후만 재정렬한다. debounce는 50ms이며 세대별 취소/소유권 검증은 유지한다.
- 열린 백틱 경로에서 Ctrl+W/Alt+Backspace 및 kitty Ctrl+Backspace는 `/` 경계에서 멈춘다.
- 스트림 EOF 전 첫 결과, 실행 파일/깨진 링크, 선택 안정성, Enter/한글/중간 커서, 경로 word delete 회귀 테스트 추가.
- 전체 Go 테스트·vet, 변경 패키지 race 검사 및 실제 Docker 경로 조회/소유권 검증 통과.

## 2026-09-13 — 정상 종료 터미널 접기와 백틱 경로 힌트

- 종료 코드 0인 셸만 자동 접기/대화 포커스 복귀. 오류 종료 패널은 유지한다.
- 백틱 뒤 `/`·`~`에서만 컨테이너 경로 overlay를 열고 닫는 백틱/Esc로 종료한다.
  일반 slash command는 건드리지 않는다. 입력은 composer에 유지하고 ↑/↓ 선택, Tab 항목 완성.
- 디렉터리별 내장 fuzzy 검색, 150ms debounce, 취소/세대 식별로 stale 응답 제외.
  소유권/remote-user 권한을 확인하고 항목 이름/type만 조회하며 명령 인수에 경로를 별도 전달한다.
- 공백·한글·중간 커서/여러 줄 문장 보존, 디렉터리 선택 후 다음 단계, 숨김 파일, 조회 한도,
  shell 특수문자 파일명, 닫는 백틱·Esc·slash command 분리, 정상/오류 종료를 회귀 테스트했다.
- 실제 Docker fixture에서 `/`와 remote-user `~/` 조회 및 타 프로젝트 접근 거부 통과.
  변경 패키지 race 검사 통과. fzf 추가 설치/주입은 하지 않았고 사용자 컨테이너는 변경하지 않았다.
- 전체 Go 테스트·vet 통과. 최소 4줄 대화 영역에서도 경로 힌트 항목을 표시하며,
  그보다 좁아 표시할 수 없는 경우 보이지 않는 힌트가 키 입력을 가로채지 않는다.
...
- 터미널 패널 위아래에 좌우 두 칸 여백의 구분선을 추가했다. 최대 24줄 PTY는 유지하며
  높이 계산, 커서·마우스 좌표와 클릭 테스트를 함께 갱신했다.
...
- 본문 최대 폭 133칸, 터미널 171칸 이상에서 왼쪽 36칸 프로젝트 패널 + 2칸 간격.
  남는 폭은 오른쪽 빈 영역으로 유지한다. fullscreen 크기는 실제 터미널 크기를 유지한다.
  accounts/모달/workflow와 standalone account picker/purge에도 본문 폭 제한을 적용했다.
- 전체 프로젝트/세션을 기존 manager 목록 API로 갱신한다. 키보드 선택/목록 스크롤,
  Ctrl+Q 패널 포커스, Esc 복귀를 지원하고 Tab 순환에서는 제외한다.
- 프로젝트 간 세션 전환은 초안을 보존하고 기존 이벤트 구독을 정리한다. 오래된 refresh가
  선택한 프로젝트를 되돌리지 않게 했으며, 선택된 프로젝트 ID를 생성/로그인 callback에 전달한다.
  최초 프로젝트의 trust-config 승인을 다른 프로젝트에 넘기지 않는다.
- 입력 포커스 스타일/IME 커서의 패널 오프셋, 마우스 좌표 변환, 좁아진 창의 포커스 복구 처리.
  CLI 업데이트만 필요하며 서버/프로토 변경은 없다.
- 검증: 전체 Go 테스트, TUI/CLI race, vet, diff 검사 통과. PTY 실제 크기 변경
  (117→143→200→63칸), 본문 제한/빈 영역, 패널 포커스·프로젝트 전환·초안 보존,
  오래된 목록 응답, 선택한 프로젝트로 workflow 전달 회귀 테스트를 추가했다.

## 2026-09-13 — 파일 칩 라벨 깜빡임 수정

- 커서 SGR 스타일이 칩 문자열을 분할하면 파일 라벨 치환이 실패하던 문제를 수정했다.
  ANSI를 제외한 가시 문자 기준으로 고유 ID가 포함된 접두사를 치환하고 원래 커서/스타일
  시퀀스는 유지한다. 메타데이터가 다음 행으로 감겨도 라벨 치환에 영향을 주지 않는다.
- 커서 위치/깜빡임 양쪽 상태, 좁은 입력 폭, 한글·결합 문자·ANSI 보존 회귀 테스트를 추가했다.
  CLI 변경이며 서버 업데이트는 필요 없다.
- 검증: `go test ./...`, `go test -race ./internal/tui`, `go vet ./...`, `git diff --check` 통과.

## 2026-09-13 — 선택 칩 직접 조작과 삭제 후 미리보기 수정

- 선택된 칩에서 t/f/d를 즉시 처리한다. f는 별도 모달 없이 비동기로 파일을 첨부하며,
  업로드 중 t/d로 취소한 전환은 늦은 응답으로 복원하지 않는다. 비선택 입력과 paste 이벤트는
  문자 그대로 처리한다. 일반 입력과 질문 Other에 공통 적용한다.
- Ctrl+P의 빈 입력 → 전체 세션 cache 대체 목록을 제거했다. 현재 입력에 남아 있는 칩만
  미리보기에 표시하며 삭제/전송된 칩이 다시 나타나지 않는다. 캐시 재삽입 i 메뉴도 제거했다.
- CLI 변경이며 서버 업데이트는 필요 없다.
- 검증: 전체 Go 테스트, TUI race, vet, diff 검사 통과. 선택 칩 직접 조작, 업로드 실패/취소,
  붙여넣은 단축키 문자, 세 가지 삭제 경로 후 Ctrl+P 회귀 테스트를 추가했다.

## 2026-09-13 — 붙여넣기 칩의 원자적 커서 이동

- 일반 입력/질문 Other에서 좌우 방향키로 칩 전체를 선택하고 다음 방향키로 칩 바깥으로 이동한다.
  선택은 연두색 배경으로 표시하며 Enter는 해당 칩의 미리보기 메뉴를 연다.
- Backspace/Delete는 인접하거나 선택된 칩 전체를 제거한다. 한글/여러 줄 초안을 보존한다.
  수직/단어 이동으로 칩 내부에 도착하는 커서도 경계로 보정한다.
- t/f/d는 메뉴에서만 동작한다. t는 원문 전송 방식 선택, i는 캐시 칩을 입력창에 삽입이며
  메뉴 조작만으로 대화를 전송하지 않는다는 설명을 추가했다. CLI 업데이트만 필요하다.
- 검증: `go test ./...`, `go test -race ./internal/tui`, `go vet ./...`, `git diff --check` 통과.
  입력/Other 이동·삭제·메뉴, 한글 다중 행 보존, 좁은 화면 하이라이트 회귀 테스트를 추가했다.
