...
대화 중 Tab/Shift+Tab은 승인 영역과 입력창 사이를 순환한다. 입력창 아래에는 quota/context만 표시한다.
...
좁은 화면에서는 입력창 위, 넓은 화면에서는 오른쪽 패널에 내용 16줄을 표시한다.
내용이 짧아도 16줄 공간을 유지하며 작은 터미널에서는 입력창을 보존하도록 높이를 줄인다.
...
바로 위 오른쪽에 빨간색 `⬤ RED`를 표시하며 점만 1초 간격으로 켜고 끈다.
...
입력창 아래에는 quota/context만 표시한다. 세션 이름과 에이전트는 왼쪽 패널에서 확인한다.
