...
r은 선택한 세션의 이름을 패널에서 편집한다. s는 선택한 세션을 중지하고,
Ctrl+X를 3초 안에 두 번 누르면 선택한 세션을 중지·삭제한다. 프로젝트 행에서는
...
Windows Terminal의 Windows 빌드도 VT 입력을 활성화해 붙여넣기 경계를 보존한다.
긴 텍스트가 여러 콘솔 읽기로 나뉘어 도착해도 입력창에는 한 번의 붙여넣기로 전달하며,
문자마다 화면을 다시 계산하지 않는다. Ctrl-Enter 구분은 Win32 입력 모드로 유지한다.
터미널이 Ctrl-V를 처리하면 cxz에는 단축키 대신 붙여넣은 텍스트가 도착한다.
터미널의 붙여넣기는 SSH에서도 같은 bracketed paste 경로를 사용하므로 원격 호스트의
클립보드 접근이 필요 없다.
...
Question은 대화 아래에 별도 공간을 확보하여 표시한다. 선택지와 설명은 줄바꿈해도
들여쓰기를 유지한다. ↑/↓ 또는 Tab으로 이동하고 Space/Enter로 선택한다. 이미
선택한 항목을 Enter로 다시 선택하면 Next/Submit에 포커스가 이동하며, 실제 제출에는
한 번 더 확인이 필요하다. 복수 선택의 Space와 마우스 클릭은 선택/해제를 반복한다.

선택지의 설명·미리보기와 빈 여백까지 클릭할 수 있으며, Other는 클릭한 뒤 직접
입력한다. 커서 항목은 테두리 안쪽 전체 너비를 ANSI 238 배경으로 채운다. 마우스
hover에도 같은 스타일을 적용하며 키보드 강조는 숨긴다. 클릭 없이 벗어나면 이전
키보드 위치를 복원하고, 클릭하면 해당 항목이 새 위치가 된다. Next/Submit, Back,
Cancel 버튼은 패널 하단에 고정하며 마우스로도 조작할 수 있다.

질문 위의 마우스 휠과 PgUp/PgDn은 긴 선택지·설명·미리보기를 스크롤한다.
대화 위의 휠 또는 Ctrl/Alt+PgUp/PgDn은 질문을 유지한 채 대화를 스크롤한다.
Ctrl+Home/End는 대화의 처음/최신 위치로 이동한다. 질문의 Home/End는 내용의
처음/끝으로 이동하며, Other 입력 중에는 텍스트 커서 이동으로 동작한다.
Esc/Cancel은 답변을 보내지 않고 닫으며, 입력창 초안과 포커스를 복원한다.
...
세션 행에서 Ctrl+X를 3초 안에 두 번 누르면 선택한 세션을 중지·삭제한다. 첫 입력은 패널
상단에 재입력 안내를 표시하며, 3초가 지나면 다시 두 번 눌러야 한다. 다른 키·클릭·스크롤,
포커스 이탈 또는 선택 대상/run 변경은 확인을 취소한다. d/Delete는 세션을 삭제하지 않는다.
삭제 RPC는 비동기로 실행하며 처리 중에도 목록 탐색·대화 열기·detach가 가능하다.
Ctrl-Q는 프로젝트·세션 탐색 화면을 열며 agent를 중지하거나 프롬프트를 전송하지 않는다.
Ctrl-D는 TUI만 닫는다.
it은 세션 화면을 선택해 열 뿐 중지된 agent를 자동 재개하지 않는다(Ctrl-R로 재개).

프로젝트 목록은 프로젝트와 그 아래 세션 행을 표시하고 선택한 행을 강조한다.
프로젝트 사이에는 얇은 어두운 회색 구분선을 표시하며, 구분선은 키보드·마우스 선택 대상에서 제외한다.
...
실행 중인 도구의 `[•] Bash` 같은 표시에서는 `•`만 0.5초 간격으로 깜빡인다.
화면에 보이는 표시만 바꾸며 대화 렌더링 캐시나 스크롤 위치는 변경하지 않는다.
...
실시간 응답은 50ms 또는 128개 이벤트/512KiB 단위로 묶어 화면에 반영한다. 마지막
묶음은 새 이벤트가 없어도 반영하고, 이벤트 순서와 원문은 그대로 보존한다. 새 응답의
Markdown·구문 강조는 수신 goroutine에서 준비하며, 완료된 입력행과 비용·토큰 요약은
재사용한다. 녹화의 `live_batch`에는 묶음 크기와 준비 시간이 들어가므로 `update`,
`transcript_render`, `render`, `terminal_write`와 비교해 지연 구간을 확인할 수 있다.

입력창 위의 `send · accepted`는 데몬이 입력 요청을 접수했다는 뜻이며, 에이전트의
작업 완료를 뜻하지 않는다. 같은 상태 줄에는 다른 명령의 처리 결과, 연결 상태,
대화 스크롤 위치, 작업 선택 안내, 백그라운드 작업 상태와 녹화 표시가 나타난다.

명령 실패는 입력창 위의 별도 오류 패널에 표시한다. 내용 영역은 3줄로 고정하고
긴 내용은 줄바꿈한다. 입력 포커스와 초안을 유지하며, Tab 또는 클릭으로 패널을
선택한 뒤 ↑/↓, PgUp/PgDn, Home/End 또는 마우스 휠로 스크롤할 수 있다.
오른쪽 위 ⧉ 또는 포커스된 상태의 Ctrl+C는 오류 원문 전체를 복사하고, × 또는
포커스된 상태의 x/Esc는 닫는다. Tab은 패널을 유지한 채 입력창으로 돌아간다.
오류는 이후의 성공 알림이나 복사 안내로 사라지지 않으며, 닫아도 `/logs`에서
원문을 확인할 수 있다. 작은 화면에서는 작업 상세보기 높이를 줄여 입력창을 보존한다.

프로젝트 목록의 세션 행은 전경·백그라운드 작업이 모두 없을 때 상태 표시를 비우고
작업 중에는 8점 점자 스피너를 표시한다. 승인·답변 대기도 진행 중에 포함한다. 작업이 끝났지만 아직
확인하지 않은 세션에는 `+`를 표시하고, 해당 대화의 최신 내용이 로딩을 마쳐
실제로 보이면 지운다. 과거 대화로 스크롤하거나 다른 패널에 포커스를 둔 상태는
읽음으로 처리하지 않는다. 읽음 상태는 연결별 세션 ID로 구분하며 현재 TUI 실행
중에만 유지한다. 상태 갱신 사이에 끝난 짧은 작업은 변경된 기록만 비동기로
확인하므로 quota·권한·에이전트 업데이트 기록을 작업 완료로 오인하지 않는다.

세션 행은 별도 들여쓰기 없이 커서와 indicator 사이 한 칸을 유지한다.
전경이 idle이어도 현재 run의 백그라운드 작업이 남아 있으면 동일한 8점 spinner를 표시한다.
열지 않은 세션도 변경된 목록 정보를 기준으로 축약된 백그라운드 상태만 비동기 조회한다.
