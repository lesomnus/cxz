...
스크롤한다. 최소 터미널 크기는 40 × 14이며 화면·입력창·테두리 배경은 검정으로 채운다.
...
Enter 실행, Esc 닫기를 지원하며 /context, /compact, /usage는 로컬 Unimplemented
응답만 표시한다. 기존 /answer와 /stop도 힌트에 포함한다. 상태바의 sessions 라벨은
제거했으며 id: 뒤의 hex 값은 세션 ID 앞 8자리다.
