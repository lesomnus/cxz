...
세션에서 `Ctrl+백틱` 또는 `/terminal`을 사용하면 하단에 최대 24줄 PTY와 1줄 헤더, 위아래 구분선이 열린다.
...
터미널 밖 헤더의 `[Back to chat]`는 패널을 유지하고 대화에 포커스를 준다.
`[Collapse ▾]`는 패널을 접는다. 입력창/터미널 본문 클릭으로도 포커스를 전환한다.
위아래 구분선은 기존 구분선처럼 좌우 두 칸을 비운다.
헤더는 내부 프로그램의 마우스 모드와 별개로 cxz가 처리한다. 터미널 포커스 중에는
Ctrl+C/Tab/Ctrl+S 등을 셸로 전달하므로 cxz 대화 단축키로 처리되지 않는다.

셸은 세션별로 하나씩 만들며 접기·세션 전환 중에도 유지된다. `exit` 후 접었다 다시 열면
새 셸을 만든다. 프로젝트의 remote user와 remote workspace로 실행하고 provider의 private
...
171칸 이상(높이 14줄 이상)이면 왼쪽에 36칸 프로젝트 패널과 2칸 간격을 추가한다.
...
프로젝트 뷰를, 세션 행에서 Enter는 해당 세션의 대화 뷰를 연다. 프로젝트 간 이동에서도
세션별 초안을 보존한다. 다른 프로젝트의 세션 생성/로그인은 그 프로젝트를 대상으로 한다.
시작 명령의 `--trust-config`는 원래 프로젝트에만 적용하며 다른 프로젝트로 전파하지 않는다.

대화 중 Tab/Shift+Tab은 기존 approvals → 입력 → sessions 순환만 사용한다.
Ctrl+Q는 패널이 보일 때 패널에 포커스를 주며 대화 뷰는 유지한다. 패널에서 Tab은 포커스를
이동하지 않고 Esc/Ctrl+Q는 원래 뷰로 돌아간다. 패널이 없는 폭에서는 Ctrl+Q가 기존처럼
프로젝트 뷰를 연다. 창을 좁혀 패널이 사라지면 패널 포커스도 해제한다.
...
`Ctrl+P`로 미리보기를 연다. `↑/↓`는 paste 선택, `PgUp/PgDn`은 미리보기 스크롤,
`t`는 원문 전송, `f`는 파일 첨부 전환, `d`는 초안에서 제거, `Esc`는 닫기다.
Ctrl+P 미리보기는 현재 입력에 남아 있는 칩만 표시한다. 삭제/전송한 칩은 캐시에서
다시 표시하지 않는다. `/paste`도 동일한 현재 입력 목록을 사용한다.
...
login`을 선택한다. 기존 세션의 재로그인은 프로젝트 뷰에서 해당 세션을 먼저 중단해야 한다.
