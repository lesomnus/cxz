...
스크롤한다. 최소 터미널 크기는 40 × 14이며 밝은/어두운 배경에 맞춘 강조색을 사용한다.
...
`>` 앞에는 화면 여백을 두지 않는다. 완료 JSON은 숨기고 응답 다음 줄에 우측 정렬된
요약을 표시한다: ↑ 입력 토큰, ↓ 출력 토큰, ↺ 캐시 읽기, ⊕ 캐시 생성, ∑ 전체 토큰,
$ USD 비용, ◷ 소요 시간. 제공되지 않은 항목은 생략하고 Codex 사용량은 tokenUsage.last만
사용한다. 제공된 소요 시간이 없으면 요청~완료 이벤트 간 시간을 ◷≈로 구분한다.
