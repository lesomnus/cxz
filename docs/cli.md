...
시간은 indicator 두 칸 뒤 바로 시작한다. 나머지 메트릭은 어두운 색의 5셀 고정 폭
...
스피너를 그린다. starting/idle/working/waiting_input/stopping/stopped/interrupted/failed가
...
Tab은 입력→승인(있을 때)→세션 선택→입력을 순환한다. 승인 포커스에서는 ↑/↓ 선택,
...
위로 완전히 사라지면 대화 최상단을 최대 2줄 덮어 고정한다. 최근 2,000개 제한을 제거해
