...
| `up [WORKSPACE]` | 기본 `.`. 프로젝트를 준비하고 전체 프로젝트 목록에서 해당 프로젝트에 포커스를 둔다. 이미 실행 중이면 재준비 없이 연결. 세션 생성/Account/로그인 요구 없음. 비대화형 또는 `--no-attach`는 프로젝트 정보 출력 |
...
최상위 up은 전체 프로젝트·세션 목록을 열고 실행 디렉터리의 프로젝트 행에 포커스를 둔다.
...
좁은 화면에서는 입력창 위에 최대 6줄, 넓은 화면에서는 오른쪽 패널에 표시한다.
작업 패널도 어두운 회색(#303030) 배경과 상하좌우 한 칸 패딩을 사용한다.
외곽 테두리 없이 포커스가 있을 때만 하단 선을 표시한다. 제목 줄의 × 또는 x/Esc로 닫는다.
