...
닫는 백틱을 입력하면 탐색이 끝난다. Esc도 닫으며 같은 입력에서는 즉시 다시 열지 않는다.
공백·한글 경로, 문장 중간/여러 줄 입력에서 주변 문장과 커서 위치를 유지한다.

150ms debounce 후 컨테이너 소유권/실행 상태를 확인하고 remote user 권한으로 한 단계의 이름과
디렉터리 여부만 조회한다. `~`는 호스트나 agent private HOME이 아니라 컨테이너 remote user의 홈이다.
