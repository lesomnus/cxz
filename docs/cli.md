...
열린 백틱 경로 안에서 `/` 경계까지만 지우고, 경계에서는 `/` 한 글자를 지운다.

50ms debounce 후 컨테이너 소유권/실행 상태를 확인하고 remote user 권한으로 한 단계의 이름,
...
검색어를 바꾸면 전체 후보를 다시 정렬한다. Docker 연결과 셸의 디렉터리 이름 열거 시간은 여전히 필요하다.
