...
(Windows는 `cxz.previous.exe`)로 보관한다. PATH의 심볼릭 링크는 유지하고 실제 대상 파일을 갱신한다.

Linux에서 `/usr/local/bin`처럼 설치 디렉터리에 직접 쓸 권한이 없으면 Docker의 root 설치
컨테이너로 자동 전환한다. 별도 `sudo`나 플래그 없이 같은 `cxz self-update`를 사용한다.
이 컨테이너에는 설치 디렉터리 하나만 쓰기 가능하게 마운트하고, 검증한 바이너리는 stdin으로
전달한다. 기존 파일의 소유자·그룹·권한을 보존하며 같은 디렉터리에서 백업과 원자적 교체를 수행한다.
설치 helper는 `python:3.13-alpine` 이미지를 사용하며 호스트 Python 설치는 필요 없다.
helper의 네트워크는 끄고 root 파일시스템은 읽기 전용으로 실행한다.

빌드 전에 현재 실행 파일과 소유권을 비교하고, 일회용 확인 파일이 **클라이언트에서도** 보이는지
검사해 Docker가 같은 설치 디렉터리를 보는지 확인한다. 다른 호스트의 동일한 경로·동일한 바이너리만으로는
검사를 통과할 수 없다. rootless Docker처럼 해당 디렉터리에 쓸 수 없는 엔진이나 설치 경로를
공유하지 않는 원격 Docker 엔진은 설치에 사용할 수 없다. 원격 엔진을 통한 빌드 자체는 계속 지원한다.
직접 설치와 Docker 설치는 같은 파일 잠금을 사용하며, 취소나 빌드 실패 시 helper를 정리한다.
Windows에서는 기존처럼 설치 디렉터리에 직접 쓸 수 있어야 한다.
