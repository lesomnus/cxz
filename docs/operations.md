...
설정은 해당 `--state`의 `settings.jsonm`에 저장한다(Linux 파일 모드 0600).
Windows 기본 경로는 `%USERPROFILE%\.local\state\cxz\settings.jsonm`이다.
...
VS Code에서는 `"files.associations": { "*.jsonm": "jsonc" }`를 **에디터 설정**에 추가한다.
기존 `settings.json`은 새 파일이 없을 때 읽고, 다음 저장부터 `settings.jsonm`으로 저장한다.
원본은 백업으로 남기며 두 파일이 모두 있으면 `settings.jsonm`을 우선한다.
